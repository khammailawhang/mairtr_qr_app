...
const fetchData = useCallback(async function() {
  ...
  // 🎯 [ເວີຊັນປົດລັອກບັກ WHERE clause 100%]: ໃສ່ເງື່ອນໄຂ .neq('id', 0) ເພື່ອຫຼອກຜ່ານດ່ານຄວາມປອດໄພ Supabase ບັງຄັບອັບເດດທຸກໂຕະບາດດຽວ
  const handleBulkTableStatus = async (targetStatus) => {
    ...
    try {
      // 🔐 [ຈຸດແກ້ໄຂຫຼັກ]: ຝັງ .neq('id', 0) ເຂົ້າໄປເພື່ອຜ່ານກົດຄວາມປອດໄພ ບັງຄັບໃຫ້ອັບເດດທຸກໂຕະໃນຮ້ານບາດດຽວສະລຸຍ 100%
      const { error } = await supabase
        .from('tables')
        .update({ status: targetStatus, bill_requested: false })
        .neq('id', 0);

      if (error) throw error;
      
      if (typeof fetchData === 'function') fetchData();
      alert('🚀 ຈັດການສະຖານະໂຕະທັງໝົດສຳເລັດຮຽບຮ້ອຍແລ້ວເຈົ້າ!');
    } catch (err) {
...
useEffect(function() {
  ...
  // 🎯 [ເວີຊັນປັບຈັງຫວະພໍດີງາມ 6 ວິນາທີ]: ດັງເຕືອນທຸກໆ 6 ວິນາທີ ສະບາຍຫູ ບໍ່ດັງຖີ່ເກີນໄປ ແລະ ມິດທັນທີເມື່ອພົ້ນສີແດງ
  useEffect(function() {
    if (!audioEnabled) return;

    // 🔐 ໑. ປັບໂມງຈັບເວລາໃຫ້ເປັນ 6 ວິນາທີ (6000ms) ເພື່ອຄວບຄຸມຈັງຫວະສຽງໃຫ້ດັງສະບາຍຫູພໍດີງາມ
    const universalAudioTimer = setInterval(function() {
      if (typeof fetchData === 'function') fetchData();
      if (typeof fetchAllData === 'function') fetchAllData();
    }, 6000);

    // 🔐 ໒. ກັ່ນຕອງຫາສະເພາະລາຍການອາຫານໃນຮ້ານ (allServerItems) ທີ່ຍັງຄົງຄ້າງສະຖານະ 'pending' (ສີແດງ) ແທ້ໆ
    const liveRedPendingItems = (allServerItems || []).filter(function(item) {
      return item && (item.item_status === 'pending' || item.status === 'pending');
    });

    // 🔐 ໓. ຕາບໃດທີ່ມີອໍເດີ້ "ສີແດງ Pending" ຄ້າງຢູ່ ໃຫ້ຫຼິ້ນສຽງເຕືອນ ໑ ຄັ້ງ ທຸກໆ 6 ວິນາທີ
    if (liveRedPendingItems.length > 0) {
      const audio = new Audio('/notification1.mp3');
      audio.play().catch(function(e) { console.log('Audio error:', e); });
    }

    return function() {
      clearInterval(universalAudioTimer);
    };
  }, [allServerItems, audioEnabled, fetchData, fetchAllData]);
