...
export default function StaffDashboardPage() {
  ...
  // 🎯 [ພາກສ່ວນທີ 1]: ເພີ່ມ State pin ແລະ ປັບປຸງ Logic ກວດເຊັກເບີໂທ + ລະຫັດ PIN 4 ຫຼັກ ປອດໄພ 100%
  const [bulkMenuOpen, setBulkMenuOpen] = useState(false);
  const [phoneNumber, setPhoneNumber] = useState("");
  const [pin, setPin] = useState(""); // State ຈື່ລະຫັດ PIN 4 ຫຼັກ
  const [isLoggedIn, setIsLoggedIn] = useState(false);
  const [isHydrated, setIsHydrated] = useState(false);
   // 🎯 [ເວີຊັນປິດບັກແຖວ 81 ຫາຍຂາດ 100%]: ຍ້າຍການອ່ານ localStorage ມາໄວ້ໃນ Lazy State Initialization ຜ່ານກົດ React 19 100%
   const [pinnedTableIds, setPinnedTableIds] = useState(function() {
    if (typeof window !== 'undefined') {
      try {
        const savedPinnedTableIds = window.localStorage.getItem("__staff_pinned_tables");
        if (savedPinnedTableIds) {
          const parsedPinnedTableIds = JSON.parse(savedPinnedTableIds);
          if (Array.isArray(parsedPinnedTableIds)) {
            return parsedPinnedTableIds.map(id => String(id));
          }
        }
      } catch (storageError) {
        console.error("Error reading pinned tables from localStorage:", storageError);
      }
    }
    return [];
  });
  ...
  const hasPendingRedItemsRef = useRef(false);
  const previousPendingItemIdsRef = useRef(null);
  const bulkMuteLockRef = useRef(false);
  ...
  const togglePinnedTable = (tableId) => {
    const normalizedTableId = String(tableId);
    setPinnedTableIds(function(currentPinnedTableIds) {
      const isPinned = currentPinnedTableIds.includes(normalizedTableId);
      const nextPinnedTableIds = isPinned
        ? currentPinnedTableIds.filter(id => id !== normalizedTableId)
        : [normalizedTableId, ...currentPinnedTableIds];

      window.localStorage.setItem('__staff_pinned_table_ids', JSON.stringify(nextPinnedTableIds));
      return nextPinnedTableIds;
    });
  };

  // 🎯 [ເວີຊັນດຶງຂໍ້ມູນຈາກ Supabase 100%]: ກວດເຊັກເບີໂທ ແລະ ລະຫັດ PIN ສົດໆຈາກຕາຕະລາງ staffs
  const handleLogin = async (e) => {
    ...
    try {
      // 🔐 ຍິງຄຳສັ່ງໄປກວດເຊັກເບີໂທ ແລະ ລະຫັດ PIN ໃນຕາຕະລາງ staffs ພ້ອມກັນ
      const { data: staffData, error: authError } = await supabase
        .from('staffs')
        .select('*')
      ...
      // ⚠️ ຫາກຖານຂໍ້ມູນຕອບກັບມາມີຕົວຕົນແທ້ (Login ສຳເລັດ)
      if (staffData) {
        if (typeof window !== 'undefined') {
          window.localStorage.setItem("__staff_logged_in", "true");
          window.localStorage.setItem("__staff_user_name", staffData.name || "");
        }
        setIsLoggedIn(true);
        alert(`🔓 ຍິນດີຕ້ອນຮັບທ່ານ ${staffData.name || 'ພະນັກງານ'}, ເຂົ້າສູ່ລະບົບສຳເລັດແລ້ວ!`);
      } else {
        alert("❌ ເບີໂທລະສັບ ຫຼື ລະຫັດ PIN 4 ຫຼັກ ບໍ່ຖືກຕ້ອງ! ກະລຸນາກວດເຊັກຄືນໃໝ່.");
      }
    } catch (err) {
      alert("❌ ເກີດຂໍ້ຜິດພາດໃນການເຊື່ອມຕໍ່ຖານຂໍ້ມູນ: " + err.message);
    }
  };


  const handleLogout = () => {
    if (window.confirm("🚪 ທ່ານຕ້ອງການອອກຈາກລະບົບພະນັກງານແທ້ບໍ່?")) {
      if (typeof window !== 'undefined') {
        window.localStorage.removeItem("__staff_logged_in");
      }
      setIsLoggedIn(false);
      setPhoneNumber("");
      setPin(""); // ເຄຼຍຄ່າ PIN ອອກ
    }
  };
  ...
  const fetchAllData = useCallback(async () => {
    try {
      ...
      const fetchedTables = (tablesRes.data || []).filter(table => table.is_available !== false);
      ...
      setBills(totals);
       ...
       // =======================================================================
      // 🚀 ⭐️ [ເວີຊັນປິດຈົບທຸກບັກ - ປ້າຍເງິນດີດກັບມາ 100% + ຕັດສຽງຊ້ອນລົວອອກ] ⭐️ 🚀
      // =======================================================================
      if (!globalThis.__prevPendingDrinksCountMap) globalThis.__prevPendingDrinksCountMap = {};

      (fetchedTables || []).forEach(table => {
        if (!table) return;
        const currentPendingDrinks = (currentServerItems || []).filter(i => Number(i.category_id) === 2 && String(table.id) === String(i.table_id) && i.item_status === 'pending').length;
        const currentServedFood = (currentServerItems || []).filter(i => Number(i.category_id) === 1 && String(table.id) === String(i.table_id) && i.item_status === 'served').length;

        globalThis.__prevPendingDrinksCountMap[table.id] = currentPendingDrinks;
        if (prevServedFoodCountRef.current) {
          prevServedFoodCountRef.current[table.id] = currentServedFood;
        }
      });

      // 🎯 [Sync ປ້າຍຮຽກເກັບເງິນ]: ຈື່ຄ່າລຳດັບປ້າຍເງິນໂຕະເກົ່າໄວ້
      const currentlyRequested = (fetchedTables || []).filter(t => 
        t && (t.bill_requested === true || t.bill_requested === 'true' || String(t.bill_requested).toLowerCase() === 'true')
      ).map(t => t.id);

      if (prevRequestedTablesRef.current) {
        prevRequestedTablesRef.current = currentlyRequested;
      }

      // 🎯 [Sync ຂໍ້ມູນໃບບິນສົດ]: ອັບເດດລາຍການອາຫານໃນໃບບິນໂຕະທີ່ກຳລັງເປີດຢູ່
      if (selectedTable) {
      ...
      // ⚡ [ຈຸດກູ້ຊີບປ້າຍເງິນ ໑]: ບັງຄັບຄຶງປ້າຍຮຽກເກັບເງິນສີເຫຼືອງ 💵 ກັບມາໂຊທັນທີ 100% ຕອນກົດເປີດໂຕະທັງໝົດ ໂດຍບໍ່ສົນ Cache
      if (prevTableBillRequestedMapRef.current && Array.isArray(fetchedTables)) {
        setTables(function(currentLatestTables) {
          if (!Array.isArray(currentLatestTables)) return currentLatestTables;
          return currentLatestTables.map(function(t) {
            var wasRequestedBefore = prevTableBillRequestedMapRef.current[t.id] === true || prevTableBillRequestedMapRef.current[Number(t.id)] === true;
            if (wasRequestedBefore) {
              return { ...t, bill_requested: true };
            }
            return t;
          });
        });
  ...
  }, [selectedTable]); // 🟢 [ເປະ 100%]: dependencies ຄົບຖ້ວນ ຜ່ານທຸກກົດ ESLint!
...
const fetchData = useCallback(async function() {
  try {
    ...
    if (tablesRes.error) throw tablesRes.error;
    setTables((tablesRes.data || []).filter(table => table.is_available !== false));
    ...
    if (!catErr && catData && catData.length > 0) {
      setCategories(catData.filter(category => category.is_available !== false));
    } else {
      setCategories([
    ...
    // ⚡ [ຈຸດກູ້ຊີບປ້າຍເງິນ ໑]: ບັງຄັບຄຶງປ້າຍຮຽກເກັບເງິນສີເຫຼືອງ 💵 ກັບມາໂຊທັນທີ 100% ຕອນກົດເປີດໂຕະທັງໝົດ ໂດຍບໍ່ສົນ Cache
    if (prevTableBillRequestedMapRef.current) {
      setTables(function(currentLatestTables) {
        if (!Array.isArray(currentLatestTables)) return currentLatestTables;
        return currentLatestTables.map(function(t) {
          var wasRequestedBefore = prevTableBillRequestedMapRef.current[t.id] === true || prevTableBillRequestedMapRef.current[Number(t.id)] === true;
  ...
  // 🎯 [ເວີຊັນປິດຈົບທຸກບັກ - ປ້າຍຮຽກເກັບເງິນດີດກັບມາ 100%]: ຝັງລະບົບ Lock Memory ປະສານງານຮ່ວມກັບ fetchData ຫາຍຂາດຖາວອນ
  const handleBulkTableStatus = async (targetStatus) => {
    const confirmMsg = targetStatus === 'occupied' 
      ? '⚠️ ທ່ານແນ່ໃຈບໍ່ວ່າຕ້ອງການ "ເປີດນຳໃຊ້ໂຕະທັງໝົດ" ໃນຮ້ານ?' 
      : '⚠️ ທ່ານແນ່ໃຈບໍ່ວ່າຕ້ອງການ "ເຄຼຍ/ປິດໂຕະທັງໝົດ" ໃຫ້ກາຍເປັນໂຕະຫວ່າງ?';
      
    if (!window.confirm(confirmMsg)) return;

    // 🔐 ໑. ປິດສຽງຊົ່ວຄາວທັງເປີດ ແລະ ປິດໂຕະທັງໝົດ ບໍ່ໃຫ້ Realtime/ປ້າຍເງິນດັງຊ້ອນ
    bulkMuteLockRef.current = true;

    // 🔐 ໒. ຈື່ປ້າຍຮຽກເກັບເງິນໄວ້ກ່ອນ ທັງເປີດ ແລະ ປິດໂຕະທັງໝົດ
    const requestedIds = [];
    if (Array.isArray(tables)) {
      tables.forEach(function(t) {
        const isRequested =
          t.bill_requested === true ||
          t.bill_requested === 'true' ||
          String(t.bill_requested).toLowerCase() === 'true';
        if (isRequested) {
          requestedIds.push(t.id);
          if (prevTableBillRequestedMapRef.current) {
            prevTableBillRequestedMapRef.current[t.id] = true;
            prevTableBillRequestedMapRef.current[Number(t.id)] = true;
          }
        }
      });
    }
    ...
    try {
      if (targetStatus === 'occupied') {
        // 🟢 ເປີດໂຕະຫວ່າງເປັນ occupied ໂດຍບໍ່ລຶບປ້າຍຮຽກເກັບເງິນ
        ...
        // 🔴 ປິດສະເພາະສະຖານະໂຕະ — ຮັກສາ bill_requested ໄວ້ຄືເກົ່າ
        const { error } = await supabase
          .from('tables')
      ...
      // ໓. ຣີເຟຣຊດຶງຂໍ້ມູນຫຼ້າສຸດ (ເຊິ່ງຈະຖືກບັງຄັບຄຶງປ້າຍເງິນຄືນມາທັນທີ 100% ຢູ່ທ້າຍຟັງຊັນ fetchData)
      if (typeof fetchData === 'function') await fetchData();
      if (typeof fetchAllData === 'function') await fetchAllData();
      
      alert('🚀 ຈັດການສະຖານةໂຕະທັງໝົດສຳເລັດຮຽບຮ້ອຍແລ້ວເຈົ້າ!');
    } catch (err) {
      alert('❌ ເກີດຂໍ້ຜິດພາດ: ' + err.message);
    } finally {
      setTimeout(function() {
        bulkMuteLockRef.current = false;
      }, 5000);
    }
  };
...
useEffect(function() {
  ...
  // Prioritize tables with new pending orders, then bill requests and occupied tables.
  const sortedTables = useMemo(() => {
    if (!tables) return [];
    return [...tables].sort((a, b) => {
      const aIsPinned = pinnedTableIds.includes(String(a.id));
      const bIsPinned = pinnedTableIds.includes(String(b.id));

      if (aIsPinned && !bIsPinned) return -1;
      if (!aIsPinned && bIsPinned) return 1;

      const aHasPending = (allServerItems || []).some(item =>
        String(item.table_id) === String(a.id) &&
        (item.item_status === 'pending' || item.status === 'pending')
      );
      const bHasPending = (allServerItems || []).some(item =>
        String(item.table_id) === String(b.id) &&
        (item.item_status === 'pending' || item.status === 'pending')
      );
  ...
  }, [tables, allServerItems, pinnedTableIds]);

  useEffect(function() {
    const pendingItems = (allServerItems || []).filter(function(item) {
      return item && (item.item_status === 'pending' || item.status === 'pending');
    });

    hasPendingRedItemsRef.current = pendingItems.length > 0;

    const currentPendingItemIds = new Set(pendingItems.map(function(item) {
      return String(item.id);
    }));
    const previousPendingItemIds = previousPendingItemIdsRef.current;

    if (audioEnabled && previousPendingItemIds !== null) {
      const hasNewPendingItem = Array.from(currentPendingItemIds).some(function(itemId) {
        return !previousPendingItemIds.has(itemId);
      });

      if (hasNewPendingItem && !bulkMuteLockRef.current) {
        const audio = new Audio('/notification1.mp3');
        audio.play().catch(function(e) { console.log('Audio error:', e); });
      }
    }

    previousPendingItemIdsRef.current = currentPendingItemIds;
  }, [allServerItems, audioEnabled]);

  // Keep one alarm loop so data refreshes cannot start overlapping sounds.
  useEffect(function() {
    if (!audioEnabled) return;

    const alarmTimer = setInterval(function() {
      if (!bulkMuteLockRef.current && hasPendingRedItemsRef.current) {
      const audio = new Audio('/notification1.mp3');
      audio.play().catch(function(e) { console.log('Audio error:', e); });
      }
    }, 6000);
  ...
  // 🎯 [ເວີຊັນປິດບັກແຖວ 490 ຫາຍຂາດ 100%]: ໃຊ້ setTimeout 0ms ດີດໄປຮອບຖັດໄປ ປ້ອງກັນບັກ Cascading Renders ຕາມກົດ React 19
  useEffect(() => {
    const hydratetimer = setTimeout(() => {
      setIsHydrated(true);
    }, 0);
  ...
  // 🎯 [ຈຸດຍ້າຍສຳຄັນ]: ຍ້າຍຟັງຊັນ prepareModalContext ຂຶ້ນມາໄວ້ດ້ານເທິງສຸດ (ກ່ອນແຖວ return ດັກຈັບທັງໝົດ) ປ້ອງກັນບັກ Syntax 100%
  ...
  // 🎯 [ເວີຊັນປິດບັກລັອກ Loading 100%]: ຈັດລຳດັບຄວາມປອດໄພໃໝ່ ບັງຄັບກວດເຊັກ Hydration ແລະ Login ກ່ອນການກວດ Loading ຫຼັງບ້ານ
  
  // ໑. ດັກຈັບ Hydration ກ່ອນສະເໝີ: ປ້ອງກັນບັກ Server/Client ແຕ້ມຜົນບໍ່ຕົງກັນ
  if (!isHydrated) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4 text-black font-black text-xs">
        ກຳລັງໂຫຼດຂໍ້ມູນລະບົບພະນັກງານ...
      </div>
    );
  }

  // 🎯 [ພາກສ່ວນທີ 2]: ຟອມເຂົ້າສູ່ລະບົບພະນັກງານ ເວີຊັນເພີ່ມຊ່ອງປ້ອນລະຫັດ PIN 4 ຫຼັກ ແບບປິດບັງຕົວເລກ 100%
  if (!isLoggedIn) {
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-4 text-black font-sans">
        <div className="bg-white p-6 rounded-3xl shadow-xl border w-full max-w-sm text-center">
          <div className="text-4xl mb-2">🔐</div>
          <h2 className="text-xl font-black text-gray-900 mb-1">ເຂົ້າສູ່ລະບົບພະນັກງານ</h2>
          <p className="text-xs text-gray-400 font-bold mb-6">ກະລຸນາປ້ອນເບີໂທ ແລະ ລະຫັດ PIN 4 ຫຼັກ</p>
          
          <form onSubmit={handleLogin} className="space-y-4 text-left">
            <div>
              <label className="text-xs font-black text-gray-700 block mb-1">ເບີໂທລະສັບ</label>
              <input 
                type="tel" 
                required
                placeholder="ຕົວຢ່າງ: 2099999999"
                value={phoneNumber}
                onChange={(e) => setPhoneNumber(e.target.value)}
                className="w-full text-sm font-mono p-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-orange-500 bg-gray-50 font-bold"
              />
            </div>

            {/* 🎯 [ຈຸດເພີ່ມໃໝ່]: ຊ່ອງປ້ອນລະຫັດ PIN 4 ຕົວເລກ ບັງຄັບໃສ່ໄດ້ສູງສຸດ 4 ໂຕ ແລະ ປິດບັງລະຫັດເພື່ອຄວາມປອດໄພ */}
            <div>
              <label className="text-xs font-black text-gray-700 block mb-1">ລະຫັດ PIN 4 ຫຼັກ</label>
              <input 
                type="password" 
                required
                maxLength={4}
                ...
                className="w-full text-sm font-mono p-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-orange-500 bg-gray-50 font-bold tracking-widest text-center"
              />
            </div>
            
            <button 
              type="submit"
              className="w-full bg-gradient-to-r from-orange-500 to-amber-500 text-white font-black text-sm py-3 rounded-xl shadow-md transition active:scale-95 flex items-center justify-center gap-1 cursor-pointer mt-2"
            >
              🔓 ເຂົ້າສູ່ລະບົບປອດໄພ
            </button>
          </form>
  ...
  // 📂 ໓. ຫຼັງຈາກພະນັກງານປ້ອນເບີໂທຜ່ານດ່ານ Login ເຂົ້າມາແລ້ວ ຈຶ່ງປ່ອຍໃຫ້ລະບົບກວດເຊັກ Loading ແລະ Error ຂອງຂໍ້ມູນ
  if (loading) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4 text-black font-black text-xs">
        ກຳລັງດຶງຂໍ້ມູນຈາກຖານຂໍ້ມູນ...
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4 text-red-600 font-black text-xs">
        ເກີດຂໍ້ຜິດພາດ: {String(error)}
      </div>
    );
  }
  
  const rUi = (typeof RECEIPT_UI !== 'undefined' && RECEIPT_UI) ? (RECEIPT_UI[receiptLang] || RECEIPT_UI.lo) : {};

  return (
    <div className="min-h-screen bg-gray-50 pb-16 max-w-5xl mx-auto shadow-sm border-x border-gray-200/60 relative">
      {/* Header Area */}
      <div className="bg-white shadow-sm px-4 sm:px-2 py-2 mb-2 flex flex-col sm:flex-row items-start sm:items-center justify-between border-b border-gray-100 rounded-b-2xl gap-2">
        <div>
      ...
      {/* 🎯 [ເວີຊັນບັງຄັບແຖວດຽວໃນໂທລະສັບ 100%]: ໃຊ້ Grid-cols-2 ບັງຄັບໃຫ້ຈອດລຽງແຖວດຽວກັນທຸກໜ້າຈໍ ປະຢັດພື້ນທີ່ສູງສຸດ */}
      <div className="grid grid-cols-2 gap-2 mb-1 w-full">
        
        {/* 🔊 ໑. ປຸ່ມຄວບຄຸມລະບົບສຽງຈິງ (ຕັດ w-full ອອກ ປ່ຽນມາໃຊ້ text-center ສະອາດຕາ) */}
           {/* 🔊 ໑. ປຸ່ມຄວບຄຸມລະບົບສຽງຈິງ (ເວີຊັນປິດບັກແຖວ 768: ໃຊ້ setAudioEnabled ໂດຍກົງ ປ້ອງກັນແອັບຫຼົ້ມ 100%) */}
           <button 
          onClick={function() {
          ...
          className={`px-3 py-2.5 rounded-xl text-xs font-black shadow-sm transition border text-center flex items-center justify-center gap-1 shrink-0 ${
            audioEnabled ? 'bg-green-500 text-white border-green-400' : 'bg-red-500 text-white animate-pulse border-red-400'
          }`}
        >
          {audioEnabled ? '🔊 ເປີດສຽງ' : '🔇 ປິດສຽງ'}
        </button>


        {/* 🧹 ໒. ປຸ່ມອັດສະລິຍະຈັດການໂຕະທັງໝົດ: ສະຫຼັບໄປມາ (Toggle) ບັງຄັບຢູ່ແຖວດຽວກັນໃນມືຖື */}
        {(() => {
          const allTables = tables || [];
          const totalCount = allTables.length;
          const occupiedCount = allTables.filter(function(t) { return t.status === 'occupied'; }).length;
          const isMostlyOccupied = occupiedCount > (totalCount / 2);

          return isMostlyOccupied ? (
            <button
              type="button"
              onClick={function() { if (typeof handleBulkTableStatus === 'function') handleBulkTableStatus('vacant'); }}
              className="bg-red-600 hover:bg-red-700 text-white font-black text-xs px-3 py-2.5 rounded-xl shadow-md transition active:scale-95 flex items-center justify-center gap-1 border border-red-500 cursor-pointer"
            >
              <span>🔴</span>
              <span className="truncate">ປິດໂຕະທັງໝົດ</span>
            </button>
          ) : (
            <button
              type="button"
              onClick={function() { if (typeof handleBulkTableStatus === 'function') handleBulkTableStatus('occupied'); }}
              className="bg-emerald-600 hover:bg-emerald-700 text-white font-black text-xs px-3 py-2.5 rounded-xl shadow-md transition active:scale-95 flex items-center justify-center gap-1 border border-emerald-500 cursor-pointer"
            >
              <span>🟢</span>
              <span className="truncate">ເປີດໂຕະທັງໝົດ</span>
            </button>
          );
        })()}
      ...
      <div className="px-3 sm:px-6 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 sm:gap-4 text-black">
        {sortedTables.map((table) => {
          ...
          const isPinned = pinnedTableIds.includes(String(table.id));
                ...
                <div className="flex items-start justify-between gap-1">
                  <div>
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      {isPinned && <span className="text-[10px]" title="ໂຕະຖືກໝາຍໄວ້">📌</span>}
                    </div>
                    <button
                      type="button"
                      onClick={() => togglePinnedTable(table.id)}
                      className={`mt-1 text-[9px] font-black px-1.5 py-0.5 rounded border transition ${isPinned ? 'bg-yellow-100 text-yellow-800 border-yellow-300' : 'bg-white/70 text-gray-500 border-gray-200 hover:bg-yellow-50 hover:text-yellow-700'}`}
                      title={isPinned ? 'ເອົາໝາຍອອກ' : 'ໝາຍໃຫ້ຢູ່ເທິງສຸດ'}
                    >
                      {isPinned ? '📌 ເອົາໝາຍອອກ' : '📍 ໝາຍໄວ້ເທິງສຸດ'}
                    </button>
                  </div>
            ...
            <div className="mt-3 space-y-1.5">
              {isOccupied || isBillRequested ? (
                <>
                  ...
                  <button onClick={function() { setSelectedTable(table); setReceiptDatetime(new Date().toLocaleString('lo-LA', { hour12: false })); setBillingModalOpen(true); }} className="w-full bg-orange-500 hover:bg-orange-600 text-white text-[11px] font-black py-2.5 rounded-xl transition shadow-sm border border-orange-600 flex items-center justify-center gap-1 active:scale-98">
                    💵 ເຊັກບິນເກັບເງິນ
                  </button>
                </>
              ) : (
                  ...
                  return (
                    <div key={item.id} className="bg-gray-50 p-3 rounded-2xl border border-gray-150 flex flex-col gap-2.5 shadow-inner text-xs font-black">
                      <div className="flex justify-between items-start gap-2">
                        ...
                        <span className="font-mono text-gray-900 font-black text-xs shrink-0 bg-gray-200 px-2 py-0.5 rounded-md">{rUi.colQty} {item.quantity}</span>
                      </div>
