...
export default function CustomerMenuPage() {
  ...
  const [bankAppPickerOpen, setBankAppPickerOpen] = useState(false);
  ...
  {/*const openBankAppChooser = () => {
    setBankAppPickerOpen(false);
    if (typeof window === 'undefined') return;

    const isAndroid = /Android/i.test(navigator.userAgent);
    const isIOS = /iPhone|iPad|iPod/i.test(navigator.userAgent);
    if (isAndroid) {
      window.location.href = 'intent://scan/#Intent;scheme=laoqr;action=android.intent.action.VIEW;end';
    } else if (isIOS) {
      window.location.href = 'laoqr://scan';
    } else {
      setBankAppPickerOpen(true);
    }
  };

  const openSpecificBankApp = (bankApp) => {
    setBankAppPickerOpen(false);
    if (typeof window === 'undefined') return;
    ...
    if (isAndroid && bankApp.androidPackage) {
      window.location.href = `intent://open/#Intent;package=${bankApp.androidPackage};end`;
    } else if (isIOS && bankApp.iosScheme) {
      window.location.href = `${bankApp.iosScheme}://`;
    } else {
      openBankAppChooser();
    }
  };
          ...
          {(() => {
            ...
            return (
              <div ref={checkoutModalRef} className="bg-white rounded-3xl w-full max-w-sm p-6 text-center shadow-xl overflow-y-auto max-h-[90vh] text-black">
                ...
                <div className="flex flex-row gap-2 w-full mb-3">
                  <button type="button" onClick={() => {
                    ...
                    <span>📋</span> <span>{bUi.copy}</span>
                                   ...
                                   <button 
                    type="button" 
                    onClick={() => setBankAppPickerOpen(true)}
                ...
                {bankAppPickerOpen && (
                  <div className="fixed inset-0 z-[60] bg-black/50 flex items-center justify-center p-4" onClick={() => setBankAppPickerOpen(false)}>
                    <div className="bg-white w-full max-w-xs rounded-2xl p-5 text-center shadow-2xl" onClick={event => event.stopPropagation()}>
                      <h3 className="font-black text-gray-900 text-base mb-2">
                        {lang === 'en' ? 'Choose a payment app' : lang === 'zh' ? '选择支付应用' : lang === 'th' ? 'เลือกแอปชำระเงิน' : 'ເລືອກແອັບຈ່າຍເງິນ'}
                      </h3>
                      <p className="text-[11px] text-gray-500 font-bold mb-3">
                        {lang === 'en' ? 'Choose your installed banking app.' : lang === 'zh' ? '选择手机中已安装的银行应用。' : lang === 'th' ? 'เลือกแอปธนาคารที่ติดตั้งในโทรศัพท์' : 'ເລືອກແອັບທະນາຄານທີ່ຕິດຕັ້ງໃນໂທລະສັບ'}
                      </p>
                      <div className="space-y-2">
                        {[
                          { name: 'BCEL One', logo: '/bank-logos/bcel-one.svg', androidPackage: 'com.bcel.bcelone', iosScheme: 'bcelone' },
                          { name: 'JDB Bank', logo: '/bank-logos/jdb.svg', androidPackage: 'com.jdb.mobilebanking', iosScheme: 'jdb' },
                          { name: 'LDB Trust', logo: '/bank-logos/ldb.svg', androidPackage: 'com.ldb.mobilebanking', iosScheme: 'ldb' },
                          { name: 'BFL BRED', logo: '/bank-logos/bfl.svg', androidPackage: 'com.bfl.bflmobile', iosScheme: 'bfl' }
                        ].map(function(bankApp) {
                          return (
                            <button key={bankApp.androidPackage} type="button" onClick={() => openSpecificBankApp(bankApp)} className="w-full bg-gray-50 hover:bg-emerald-50 text-gray-800 border border-gray-200 hover:border-emerald-300 font-black text-xs rounded-xl py-2.5 shadow-sm transition flex items-center gap-2 text-left px-3">
                              <img src={bankApp.logo} alt={`${bankApp.name} logo`} className="w-8 h-8 rounded-lg object-contain shrink-0" />
                              <span>{bankApp.name}</span>
                              <span className="ml-auto text-[10px] text-emerald-600">ເປີດ</span>
                            </button>
                          );
                        })}
                      </div>
                      <button type="button" onClick={openBankAppChooser} className="w-full mt-3 bg-emerald-600 hover:bg-emerald-700 text-white font-black text-xs rounded-xl py-2.5 shadow-sm transition">
                        📲 {lang === 'en' ? 'Show other compatible apps' : lang === 'zh' ? '显示其他兼容应用' : lang === 'th' ? 'แสดงแอปอื่นที่รองรับ' : 'ສະແດງແອັບອື່ນທີ່ຮອງຮັບ'}
                      </button>
                      <button type="button" onClick={() => setBankAppPickerOpen(false)} className="w-full mt-2 bg-gray-100 hover:bg-gray-200 text-gray-700 font-black text-xs rounded-xl py-2 transition">
                        {bUi.close}
                      </button>
                    </div>
                  </div>
                ...
                <div className="space-y-2 w-full mb-4">
                  ...
                  <div className="flex flex-row gap-2 w-full pt-0.5">
                    ...
                    {/* 🌍 ປຸ່ມປິດ ❌ */}
                    <button type="button" onClick={() => setCheckoutOpen(false)} className="w-1/2 bg-red-600 hover:bg-red-700 text-white border border-red-700 font-black text-xs rounded-xl py-2 transition text-center active:scale-95 shadow-sm truncate px-1">
                      {bUi.close}
                    </button>
                  </div>
                </div>
